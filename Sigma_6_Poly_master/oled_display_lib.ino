/*
  ...
  |
  |  Platform:      This version edited to suit SSD1109 OLED controller and Arduino IDE.
  |                 SAMD21 library is needed for the "Wire" class IIC driver functions.
  |
  |  Originated:    2021,  M.J. Bauer  (to suit SH1106)
  |  Revised:       2025,  M.J. Bauer  (to suit SSD1109)           [www.mjbauer.biz]
*/
#include <Wire.h> 
...
void  Disp_BlockFill(uint16_t w, uint16_t h)
{
  ...
  SSD1309_WriteBlock((uint16_t *) screenBuffer, x, y, w, h);
}
...
void   Disp_PutChar16(uint8_t uc)
{
  short  exceptions = sizeof(NotBoldCharList);  // bytes
  short  i, j, k;
...
//===============  SSD1309 OLED CONTROLLER DRIVER FUNCTIONS (IIC)  ==================

/*----------------------------------------------------------------------------------
  |  Name         :  SSD1309_WriteCommand()
  |  Function     :  Write 8-bit command to SSD1309 controller
...
void  SSD1309_WriteCommand(uint8_t cmd)
{
  ...
  Wire.write(SSD1309_MESSAGETYPE_COMMAND);
  Wire.write(cmd);
...
/*----------------------------------------------------------------------------------
  |  Name         :  SSD1309_WriteData()
  |  Function     :  Write 8-bit data to SSD1309 controller
...
{
  Wire.beginTransmission(SSD1309_I2C_ADDRESS);
  Wire.write(SSD1309_MESSAGETYPE_DATA);
  Wire.write(data);
  Wire.endTransmission();
}
...
/*----------------------------------------------------------------------------------
  |  Name         :  SSD1309_Init()
  |  Function     :  Initialize SSD1309 controller.
  |  Input        :  --
  |  Returns      :  TRUE if SSD1309 device responds wth ACK, else FALSE
  |  Note         :  SSD1309 GDRAM not cleared
*/
bool  SSD1309_Init(void)
{
  ...
  SSD1309_WriteCommand(SSD1309_DISPLAYOFF);            // 0xAE
  SSD1309_WriteCommand(SSD1309_SETDISPLAYCLOCKDIV);    // 0xD5
  SSD1309_WriteCommand(0xF0);
  SSD1309_WriteCommand(SSD1309_SETMULTIPLEX);          // 0xA8
  SSD1309_WriteCommand(0x3F);
  SSD1309_WriteCommand(SSD1309_SETDISPLAYOFFSET);      // 0xD3
  SSD1309_WriteCommand(0x00);
  SSD1309_WriteCommand(SSD1309_SETSTARTLINE + 0);      // 0x40 + line#
  // SSD1309_WriteCommand(SSD1309_CHARGEPUMP);         // 0x8D
  // SSD1309_WriteCommand(0x10);
  SSD1309_WriteCommand(SSD1309_MEMORYMODE);            // 0x20
  SSD1309_WriteCommand(0x00);
  SSD1309_WriteCommand(SSD1309_SEGREMAP | 1);          // 0xA1
  // SSD1309_WriteCommand(SSD1309_COMSCANINC);         // 0xC0
  SSD1309_WriteCommand(SSD1309_COMSCANDEC);            // 0xC8
  SSD1309_WriteCommand(SSD1309_SETCOMPINS);            // 0xDA
  SSD1309_WriteCommand(0x12);
  SSD1309_WriteCommand(SSD1309_SETCONTRAST);           // 0x81
  SSD1309_WriteCommand(30);
  SSD1309_WriteCommand(SSD1309_SETPRECHARGE);          // 0xD9
  SSD1309_WriteCommand(0xF1);
  SSD1309_WriteCommand(SSD1309_SETVCOMDETECT);         // 0xDB
  SSD1309_WriteCommand(55);
  SSD1309_WriteCommand(SSD1309_DISPLAYALLON_RESUME);   // 0xA4
  SSD1309_WriteCommand(SSD1309_NORMALDISPLAY);         // 0xA6
  SSD1309_WriteCommand(SSD1309_DISPLAYON);             // 0xAF
...
void  SSD1309_SetContrast(uint8_t level_pc)
{
  ...
  SSD1309_WriteCommand(SSD1309_SETCONTRAST);
  SSD1309_WriteCommand(level_pc * 2);
}


/*----------------------------------------------------------------------------------
  |  Name         :  SSD1309_ClearGDRAM()
  |
  |  Function     :  Clear entire SSD1309 controller graphics data RAM.
  |                  MCU RAM screen buffer is not cleared. (Use Disp_ClearScreen())
  |
  |  GDRAM format in SSD1309 controller (132 x 64 pixels):
  |  ````````````````````````````````````````````````````
  |                0 -------------- Segment --------------- 131
  |       Page 0   ||||||||||||||||||||||||||||||||||||||||||||
  |       Page 1   ||||||||||||||||||||||||||||||||||||||||||||
...
void  SSD1309_ClearGDRAM(void)
{
  ...
  for (page = 0;  page < 8;  page++)
  {
    ...
    SSD1309_WriteCommand(SSD1309_PAGEADDR + page);

    for (partition = 0;  partition < 9;  partition++)
    {
      // Set column/segment address for this partition
      SSD1309_WriteCommand(SSD1309_SETCOLUMNADDRLOW);  // AddrLow = 0
      SSD1309_WriteCommand(SSD1309_SETCOLUMNADDRHIGH + partition);
...
/*----------------------------------------------------------------------------------
  |  Function   :  SSD1309_WriteBlock()
  |
  |  Overview   :  Copies a rectangular block of pixels from an external screen
  |                buffer in MCU RAM to the SSD1309 controller GDRAM (graphics data RAM).
...
{
  uint8_t   page;                   // page address in SSD1309 (1 page = 8 rows)
  ...
  {
    SSD1309_WriteCommand(SSD1309_PAGEADDR | page);   // set page address (0..7)
    ...
    for (col = firstCol;  col <= lastCol && col < 8;  col++)
    {
      ...
      pBuf = scnBuf + offset;
      for (row = 0;  row < 8;  row++)  // 8 rows in SSD1309 page
      {
        pixelWord[row] = *pBuf;
      ...
      segAddr = col * 16;
      SSD1309_WriteCommand(SSD1309_SETCOLUMNADDRLOW + (segAddr & 0xF));
      SSD1309_WriteCommand(SSD1309_SETCOLUMNADDRHIGH + (segAddr >> 4));
      // Write bunch of pixels, 16 (H) x 8 (V) to SSD1309 controller
      Wire.beginTransmission(SSD1309_I2C_ADDRESS);
      Wire.write(SSD1309_MESSAGETYPE_DATA);
      ...
      for (x_coord = 0;  x_coord < 16;  x_coord++)
      {
        ...
        // write 8 pixels (vertical) to SSD1309 controller
...
/*----------------------------------------------------------------------------------
  |  Name         :  SSD1309_Test_Pattern()
  |
  |  Function     :  Low-level test of SSD1309 graphics write sequence.
  ...
  |  Input        :  --
  |  Note         :  May be called without first calling SSD1309_ClearGDRAM(),
